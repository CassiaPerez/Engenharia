...
const ServiceManager: React.FC<Props> = ({ services, setServices }) => {
  ...
  const [formData, setFormData] = useState<Partial<ServiceType>>({ costType: ServiceCostType.HOURLY, unitValue: 0 });
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    setShowModal(false);
    setFormData({ costType: ServiceCostType.HOURLY, unitValue: 0 });
  };
  ...
  return (
    <div className="space-y-8">
      <header className="flex justify-between items-center border-b border-slate-200 pb-6">
        <div><h2 className="text-3xl font-bold text-slate-800 tracking-tight">Catálogo de Serviços</h2><p className="text-slate-500 text-base mt-1">Atividades padrão para apontamento.</p></div>
